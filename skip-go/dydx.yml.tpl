...
coingecko:
  ...
  api_key: {{ with secret "solvers/skip-go-dydx"}}{{ .Data.data.coingecko_api_key }}{{ end }}
  requests_per_minute: 2
  cache_refresh_interval: "15m"

order_filler:
  worker_count: {{ key "networks/skip-go-dydx/base.worker_count" }}
...
{{ if (key "networks/skip-go-dydx/rebalance.enable" | parseBool)}}
...
fund_rebalancer:
{{- range $network, $job_config := (key "networks/skip-go-dydx/chain_config" | parseJSON) -}}
{{- if eq $network "ethereum" }}
{{ key "networks/skip-go-dydx/rebalance.eth" }}
{{- end -}}
{{ if eq $network "arbitrum"}}
{{ key "networks/skip-go-dydx/rebalance.arbitrum" }}
{{- end -}}
{{ if eq $network "avalanche"}}
{{ key "networks/skip-go-dydx/rebalance.avalanche" }}
{{- end -}}
{{ if eq $network "polygon"}}
{{ key "networks/skip-go-dydx/rebalance.polygon" }}
{{- end -}}
{{ if eq $network "base"}}
{{ key "networks/skip-go-dydx/rebalance.base" }}
{{- end -}}
{{ if eq $network "optimism"}}
{{ key "networks/skip-go-dydx/rebalance.optimism" }}
{{- end -}}
{{ if eq $network "osmosis"}}
{{ key "networks/skip-go-dydx/rebalance.osmosis" }}
{{- end -}}
{{- end }}
{{- end }}
...
{{ if eq $network "osmosis" }}
  osmosis-1:
    ...
    cosmos:
      ...
      gas_denom: "uosmo"
      only_fill_dydx_orders: {{ key "networks/skip-go-dydx/only_fill_dydx_orders" | parseBool }}
    relayer:
