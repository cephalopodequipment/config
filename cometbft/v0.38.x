...
proxy_app = "tcp://{{ envOrDefault "NOMAD_IP_abci" "127.0.0.1" }}:{{ keyOrDefault  (print (env "CONSUL_PATH") "/base.proxy_app") "26658" }}"
...
flush_throttle_timeout = {{ keyOrDefault (print "networks/" (index (env "CONSUL_PATH" | split "/") 1) "/p2p.flush_throttle_timeout") "\"100ms\"" }}
...
recheck_timeout = {{ keyOrDefault (print (env "CONSUL_PATH") "/mempool.recheck_timeout") "\"1s\"" }}
...
peer_gossip_sleep_duration = {{ keyOrDefault (print (env "CONSUL_PATH") "/consensus.peer_gossip_sleep_duration") "\"100ms\"" }}
